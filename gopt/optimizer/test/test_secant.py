...
from gopt.optimizer.path_point import PathPoint
from gopt.optimizer.secant import secant, secant_1, secant_2, secant_3
...
class TestSecant(unittest.TestCase):
    def setUp(self):
        ...
        with path("gopt.optimizer.test.data", "water_old.fchk") as fchk_file1:
        ...
        with path("gopt.optimizer.test.data", "water_new.fchk") as fchk_file2:
    ...
    def test_secant_condition(self):
        ...
        part2 = np.dot(self.newp.vspace.T, np.linalg.pinv(self.newp.b_matrix.T))
        part3 = np.dot(
            self.newp.b_matrix.T,
            np.dot((self.newp.vspace - self.oldp.vspace), self.newp.v_gradient),
        )
        part4 = np.dot(
            (self.newp.b_matrix - self.oldp.b_matrix).T, self.newp.q_gradient
        )
    ...
    def test_secant_condition_0(self):
        d_s = np.dot(
            self.old_ob.vspace.T, self.new_ob.ic_values - self.old_ob.ic_values
        )
