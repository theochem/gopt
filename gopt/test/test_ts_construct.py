...
from gopt.errors import InvalidArgumentError
from gopt.internal import Internal
from gopt.path_ri import PathRI
from gopt.reduced_internal import ReducedInternal
from gopt.ts_construct import TSConstruct
...
class Test_TS_Construct(unittest.TestCase):
    ...
    def setUp(self):
        with path("gopt.test.data", "ch3_hf.xyz") as rct_path:
    ...
    def test_auto_internal(self):
        ...
        assert len(ts_ins.reactant.ic_values) - len(self.reactant_ic.ic_values) == 11
    ...
    def test_ts_construct(self):
        ...
        assert_allclose(result.ic_values[:16], result_2.ic_values[:16], atol=1e-3)
        ts_ins.select_key_ic(1)
        assert ts_ins.key_ic_counter == 1
        assert np.allclose(ts_ins.ts.ic_values[:2][::-1], result_2.ic_values[:2])
        assert np.allclose(
            ts_ins.ts._cc_to_ic_gradient[1], result_2._cc_to_ic_gradient[0]
        )
    ...
    #     with path('gopt.test.data', 'ch3_hf.xyz') as rct_p:
    #         with path('gopt.test.data', 'ch3f_h.xyz') as prd_p:
    #             ts = TSConstruct.from_file(rct_p, prd_p)
    #     ts_ins = TSConstruct(self.reactant_ic, self.product_ic)
    #     ts.auto_generate_ts()
    #     ts_ins.auto_generate_ts()
    #     with path('gopt.test.data', 'ts_nose_test_cons.xyz') as filepath:
    ...
    def test_from_file_to_path(self):
        with path("gopt.test.data", "rct.xyz") as rct_path:
            with path("gopt.test.data", "prd.xyz") as prd_path:
