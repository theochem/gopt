...
from gopt.coordinate_types import (
    BendAngle,
    BendCos,
    BondLength,
    ConventionDihedral,
...
from gopt.molmod import bend_angle
...
class Test_Coordinates_Types(TestCase):
    ...
    def setup_class(self):
        with path("gopt.test.data", "methanol.xyz") as file_path:
            mol = Utils.load_file(file_path)
        self.molecule = mol

        with path("gopt.test.data", "h2o2.xyz") as file_path2:
    ...
    def test_bend_angle(self):
        ...
        # calculate ref dd
        ref_d2 = (
            -2 * (np.cos(angle.value - 1e-6) - np.cos(1.8)) * np.sin(angle.value - 1e-6)
        )
    ...
    def test_dihed_angle(self):
        ...
        # finite diff for h
        ref_d1 = (
            sin_ang1 * sin_ang2 * 2 * np.sin(dihed_angle.value - dihed_angle.target)
        )
        ...
        ref_d2 = (
            ...
            * sin_ang2
            * 2
            * np.sin(dihed_angle.value - 1e-6 - dihed_angle.target)
        )
