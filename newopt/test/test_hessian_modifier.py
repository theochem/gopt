...
class TestHessianModifier(object):
    ...
    def test_hessian_modifier_all_positive(self):
        ...
        answer = SaddleHessianModifier.modify_hessian(self.sym_mt, key_ic_number=3)
    ...
    def test_hessian_modifier_one_negative(self):
        w, v = np.linalg.eigh(self.sym_mt)
        anwser = SaddleHessianModifier.modify_hessian(self.sym_mt, negative_eigen=1, key_ic_number=3)
    ...
    def test_hessian_modifier_two_negative(self):
        w, v = np.linalg.eigh(self.sym_mt)
        anwser = SaddleHessianModifier.modify_hessian(self.sym_mt, negative_eigen=2, key_ic_number=3)
        a_w, a_v = np.linalg.eigh(anwser)
    ...
    def test_hessian_modifier_reduce_to_one_nagetive(self):
        ...
        print w, v
        anwser = SaddleHessianModifier.modify_hessian(sym_mt, negative_eigen=1, key_ic_number=3)
        a_w, a_v = np.linalg.eigh(anwser)
