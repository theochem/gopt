...
class TestInternal(object):
    ...
    def test_angle_add(self):
        ...
        mol.add_bond(0, 1)
        mol.add_bond(1, 2)
        connected_index = mol.connected_indices(1)
        assert np.allclose(connected_index, np.array([0, 2]))
        mol.add_angle_cos(0, 1, 2)
        assert len(mol.ic) == 3
        assert np.allclose(mol.ic[2].value, -0.33333406792305265)
        assert np.allclose(mol._cc_to_ic_gradient[2], np.array(
            [-0.3000493, -0.42433414,  0., 0.,  0.84866827, -0., 0.3000493, -0.42433414,  0.]))
    ...
    def test_dihedral_add(self):
        ...
        assert np.allclose(internal._cc_to_ic_hessian[
                           3][:12, :12], ref_hessian)
    ...
    def test_cost_function(self):
        ...
        mol.add_angle_cos(0, 1, 2)
        assert np.allclose(
            mol.ic_values, [1.8141372422079882, 1.8141372422079882, -0.33333406792305265])
        ...
        ref_x_hessian = np.dot(
            np.dot(mol._cc_to_ic_gradient.T, dd), mol._cc_to_ic_gradient)
        ...
        assert np.allclose(
            mol.ic_values, [1.7484364736491811, 1.7484364736491811, -0.28229028459335431])
        assert np.allclose(mol._cc_to_ic_gradient[0, :6], np.array(
