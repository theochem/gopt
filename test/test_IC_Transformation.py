...
def test_IC_Transformation_water_add_ics():
    ...
    diff = water.ic_differences - [1.8, 1.8, 0.]
...
def test_IC_Transformation_h2_azirine():
    ...
    diff = h2a.ic - np.array([2.87966936,2.67197956,2.35391472,2.05820441,2.03738343,2.03749261,
    ...
    diff = h2a.target_ic - np.array([2.7, 2.5, 2.3, 2.0, 1.8, 1.7, 1.0, 0.7, 2.7,1.99497291,1.99482514,0.04706454,0.        ])
    assert np.dot(diff, diff) < 10e-15

    diff = h2a.ic_differences - np.array([0.17966936,0.17197956,0.05391472,0.05820441,0.23738343,0.33749261,0.21448386,
    ...
    diff = h2a.target_ic - np.array([2.7, 2.5, 2.3, 2.0, 1.8, 1.7, 1.0, 0.7, 2.7,1.99497291,1.99482514,0.04706454,0.        ,-2.44151252])
    assert np.dot(diff, diff) < 10e-15
