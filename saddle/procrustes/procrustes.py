...
class Procrustes(object):
    def __init__(self, target, *candidates):
        ...
        assert all(
            np.array_equal(target.numbers, i.numbers) for i in candidates)
        self._target = target
        self._candidates = candidates
        self._target_center = Procrustes._barycenter(target.coordinates,
                                                     target.numbers)
    ...
    def rotate_mols(self):
        adj_tar_coor = Procrustes._move_to_center(self._target.coordinates,
                                                  self._target.numbers)
        for i in self._candidates:
            adj_center = Procrustes._move_to_center(i.coordinates, i.numbers)
            adj_can_coor = Procrustes._rotate_coordinates(
                adj_tar_coor, adj_center)
