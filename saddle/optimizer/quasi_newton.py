...
class QuasiNewton:
    ...
    def simple_rank_one(hes, *_, sec_y, step):
        QuasiNewton._verify_type(hes, sec_y, step)
        p1 = sec_y - dot(hes, step)
        numer = dot(p1, step)**2
        denor = dot(norm(p1)**2) * norm(step)**2
        if numer / denor <= 1e-18:
            return hes.copy()
    ...
    def bofill(hes, *_, sec_y, step):
        ...
        ratio = numer / denor
        sr1_r = QuasiNewton.sr1(hes, sec_y=sec_y, step=step)
        psb_r = QuasiNewton.psb(hes, sec_y=sec_y, step=step)
