...
class TestTrustRadius(TestCase):
    ...
    def test_trim(self):
        step = trust_region_image_potential(self.hessian, self.gradient, 0.02)
        assert np.linalg.norm(step) - 0.02 < 1e-7
        step = trust_region_image_potential(self.hessian, self.gradient, 0.1)
        assert np.linalg.norm(step) - 1 < 1e-7
        step = trust_region_image_potential(self.hessian, self.gradient, 683)
        assert np.linalg.norm(step) - 682.95203408 < 1e-7

    def test_trim_one_neg(self):
        hessian = np.diag([-1, 3, 5, 7, 9])
        gradient = np.arange(5)
        step = trust_region_image_potential(hessian, gradient, 0.766881021)
    ...
    def test_trim_all_pos(self):
        ...
        step = trust_region_image_potential(hessian, gradient, 0.8451898886)
