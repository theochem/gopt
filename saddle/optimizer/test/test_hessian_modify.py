...
from saddle.optimizer.hessian_modify import modify_hessian, modify_hessian_with_pos_defi
...
class TestHessianModify(TestCase):
    ...
    def test_one_neg(self):
        ...
        assert np.allclose(
            n_vectors[:, 0] ** 2,
            np.array([0.69293252, 0.38870599, -0.41219818, -0.24030381, -0.37563137])
            ** 2,
        )
        ...
        assert np.allclose(
            n_vectors[:, 0] ** 2,
            np.array([0.29402663, -0.24170677, -0.35568675, 0.84128791, 0.14438752])
            ** 2,
        )
        ...
        assert np.allclose(
            n_vectors[:, 0] ** 2,
            np.array([0.41754632, -0.46959165, -0.08110722, -0.42598055, 0.64583353])
            ** 2,
        )
    ...
    def test_multi_neg(self):
        ...
        assert np.allclose(
            n_vectors[:, 0] ** 2,
            np.array([0.26951539, 0.63812477, 0.48589596, 0.21054014, 0.48962863]) ** 2,
        )
    ...
    def test_mody_hes_with_pos_defi(self):
        ...
        np.allclose(
            values,
            [-0.005, 6.70424398e-02, 1.74207191e-01, 6.29035498e-01, 5.86943965e00],
        )
